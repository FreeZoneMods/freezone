...
CRestrictions = packed record
  ...
  _unused1:byte;
  _unused2:word;
  m_goups:array[0..11] of byte; {xr_map<shared_str>}
...
var
  ...
  CRestrictions__GetItemGroup:srcECXCallFunctionWEDIArg;
  CRestrictions__GetGroupCount:srcECXCallFunction;
...
begin
  ...
  CRestrictions__GetItemGroup.Call([g_mp_restrictions, @out_str, @sect_str]);
...
begin
  ...
  if xrGameDllType()=XRGAME_SV_10006 then begin
    g_mp_restrictions:=pointer(xrGame+$560AE8);
    CRestrictions__InitGroups:=srcECXCallFunction.Create(pointer(xrGame+$443f40), [vtPointer], 'CRestrictions', 'InitGroups');
    CRestrictions__GetItemGroup:=srcECXCallFunctionWEDIArg.Create(pointer(xrGame+$444460), [vtPointer, vtPointer, vtPointer], 'CRestrictions', 'GetItemGroup');
    CRestrictions__GetGroupCount:=srcECXCallFunction.Create(pointer(xrGame+$444530), [vtPointer, vtPointer], 'CRestrictions', 'GetGroupCount');

    get_rank:=srcCdeclFunction.Create(pointer(xrGame+$443ce0), [vtPointer], 'get_rank');
    result:=true;
  end;
end;
