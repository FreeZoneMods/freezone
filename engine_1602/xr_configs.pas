...
implementation
uses basedefs, windows, sysutils, CommonHelper;
...
begin
  result:=false;
  ppSettings:=GetProcAddress(xrCore,'?pSettings@@3PBVCInifile@@B');
  CInifile__r_string:=srcECXCallFunction.Create(GetProcAddress(xrCore, '?r_string@CInifile@@QBEPBDPBD0@Z'), [vtPointer, vtPChar, vtPChar], 'r_string', 'CInifile');
  CInifile__line_exist:=srcECXCallFunction.Create(GetProcAddress(xrCore, '?line_exist@CInifile@@QBEHPBD0@Z'), [vtPointer, vtPChar, vtPChar], 'line_exist', 'CInifile');
