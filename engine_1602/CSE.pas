...
const
  ...
  //$14 - for set_name_replace
  CSE_Abstract__visual_index:cardinal= $24;
...
begin
 if xrGameDllType()=XRGAME_1602 then begin
   CSE_Visual__set_visual:=srcESICallFunctionWEAXArg.Create(pointer(xrGame+$3b4c90), [vtPointer, vtPChar], 'set_visual', 'CSE_Visual');
 end;
 CSE_Abstract__visual:=srcVirtualBaseFunction.Create(CSE_Abstract__visual_index, [vtPointer], 'visual', 'CSE_Abstract'); //CHECK IT BEFORE USING!
