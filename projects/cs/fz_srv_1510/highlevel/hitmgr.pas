...
begin
  ...
  if idx<0 then begin
    ...
    _cached_ammo_restrictions[idx].max_hit_critical:=game_ini_read_float_def(section, 'k_hit_critical', -1) * k_gravity;
...
begin
  ...
  for i:=0 to items_count_in_vector(@pwpn.m_ammoTypes, sizeof(shared_str))-1 do begin
    ...
    if abs(cached_restr.max_ap - hit.armor_piercing) < EPS then begin
      ...
      if not result then begin
        ...
        if restrictions.max_hit_critical < cached_restr.max_hit_critical then restrictions.max_hit_critical := cached_restr.max_hit_critical;
...
begin
  if FZLogMgr.Get.IsSeverityLogged(FZ_LOG_DBG) then begin
    ...
    FZLogMgr.Get.Write('ap: '+FZCommonHelper.FloatToString(hit.armor_piercing, 8, 4)+'('+FZCommonHelper.FloatToString(restrictions.max_ap, 8, 4)+')', FZ_LOG_DBG);
    ...
    FZLogMgr.Get.Write('crit: '+FZCommonHelper.FloatToString(hit.power_critical, 8, 4)+'('+FZCommonHelper.FloatToString(restrictions.max_hit_critical, 8, 4)+')', FZ_LOG_DBG);
  ...
  result:= (hit.hit_type = restrictions.hit_type) and
           (abs(hit.armor_piercing - restrictions.max_ap) < EPS) and
           (hit.impulse <= restrictions.max_impulse+EPS) and
           (hit.power <= restrictions.max_hit+EPS) and
           (hit.power_critical <= restrictions.max_hit_critical+EPS);
end;
...
begin
  ...
  if pknife <> nil then begin
    ...
    if pknife.fvHitPowerCritical_1[difficulty_gdMaster] > pknife.fvHitPowerCritical_2[difficulty_gdMaster] then restrictions.max_hit_critical:=pknife.fvHitPowerCritical_1[difficulty_gdMaster] else restrictions.max_hit_critical:=pknife.fvHitPowerCritical_2[difficulty_gdMaster];
    ...
    if not _DetermineMaxAmmoRestrictions(pwpn, hit, restrictions) then begin
      FZLogMgr.Get.Write('FZHitMgr._CheckHitUsingEngineInfo: cannot find ammo with k_ap='+FZCommonHelper.FloatToString(hit.armor_piercing), FZ_LOG_DBG);
      exit;
    end;
    ...
    restrictions.max_impulse:=restrictions.max_impulse * pwpn.base_CShootingObject.fHitImpulse * pwpn.base_CShootingObject.cur_silencer_koef.hit_impulse;
    restrictions.max_hit:=restrictions.max_hit * pwpn.base_CShootingObject.fvHitPower[difficulty_gdMaster] * pwpn.base_CShootingObject.cur_silencer_koef.hit_power;
    restrictions.max_hit_critical:=restrictions.max_hit_critical * pwpn.base_CShootingObject.fvHitPowerCritical[difficulty_gdMaster];
  end else begin
