...
uses syncobjs, basedefs, xr_debug, LogMgr, Servers, sysutils, strutils, Players, xrstrings, dynamic_caster, Console, games;
...
begin
  ...
  if (length(str)=0) or (leftstr(id_str, length(str))=str) then begin
    ...
    FZReportPlayersCommand(parameter2).Report(id_str+' : ' + get_string_value(@cl.name), false);
